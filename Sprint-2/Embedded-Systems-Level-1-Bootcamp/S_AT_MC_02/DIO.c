...
#define DIO_PULL_UP_RESISTOR_ENABLE					2u
...
enuErrorStatus_t DIO_Init(void)
{
	...
	for(u8LoopIndex = 0; ((u8LoopIndex < DIO_GROUPS_NO) && (u8ErrorFlag == TRUE)); u8LoopIndex++)
	{
		...
		if((pstrConfig->enuPinNo < DIO_PINS_NO) && (pstrConfig->enuPinDir < DIO_DIRECTION_STATES) && (pstrConfig->enuPullupResEn < DIO_PULL_UP_RESISTOR_ENABLE))
		{
