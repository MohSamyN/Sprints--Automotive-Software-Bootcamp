...
enuErrorStatus_t  BT_GetState(uint8_t *au8BTState)
{
	enuErrorStatus_t enuErrorStatus;
	enuErrorStatus = BT_SendCMD((uint8_t *)"AT+STATE?\r\n",30);
	if(enuErrorStatus == E_OK)
	{
		au8BTState = gau8RxBuffer;
	}
	return enuErrorStatus;
}
...
enuErrorStatus_t  BT_Test(void)
{
   ...
   enuBTBaudRate_t enuBaudrate=BT_BR_9600;
	DIO_Init(&astrDIOConfigParameters[BT_EN]);
	DIO_Write(BT_EN, HIGH);
...
enuErrorStatus_t  BT_Init(void)
{
	...
	if (current_Baudrate != gau32Baudrates[BT_BR_38400])
	{
		enuErrorStatus = BT_SendCMD((uint8_t *)"AT+UART=38400,0,0\r\n",30);
		if (enuErrorStatus==E_OK)
		{
